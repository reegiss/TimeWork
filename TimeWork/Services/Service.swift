...
class Service {
    ...
    func request(httpMethod: HTTPMethod, url: URL, payload: [String: Any]? = nil, headers: [String:String]? = nil, auth: Bool = true) -> URLRequest {
        var request = URLRequest(url: url)
...
extension URLRequest {
    ...
    private func mapErrors(statusCode: Int, error: Error?, serviceResponse: inout ServiceResponse) {
        ...
        case 500...599:
            serviceResponse.serviceError = ServiceError(code: nil, title: "Server Error", detail: nil, statusCode: statusCode, error: "ServerError", message: "Serviço indisponível. Por favor, volte mais tarde para finalizar o registro.")
        default:
            setUnknowErrorFor(serviceResponse: &serviceResponse)
        }
    }
    ...
    func response(succeed success: @escaping (ServiceResponse) -> Void, failed failure: @escaping (ServiceResponse) -> Void,  completed completion: @escaping () -> Void) {
    ...
    func response<SuccessObjectType: Codable>(succeed success: @escaping (SuccessObjectType?, ServiceResponse) -> Void, failed failure: @escaping (ServiceResponse) -> Void,  completed completion: @escaping () -> Void) {
